...
async function main () {
    ...
    try {
        ...
        const processeddata = fs.readFileSync('src/payoutsWithTxHash.csv', 'utf8');
        const processedCsvData = processeddata.split("\n");
        ...
        const airdropAccount = keyring.addFromSeed(seed);
        let nonce = await api.rpc.system.accountNextIndex(airdropAccount.address);

        // need to append the header only once
        ...
        for (let records = 1; records < csvData.length; records++) {
            ...
            const recordAlreadyProccessed = processedCsvData.find(data => data.startsWith(` ${receiver}`));
            if (!recordAlreadyProccessed) {
