...
async function main () {
    ...
    let start = new Date("03/02/2022");
    let end = new Date("03/31/2022");
    ...
    const methods = {
        "genericAsset": ["burn", "create", "createReserved", "mint", "transfer", "transferAll", "updateAssetInfo", "updatePermission"],
        "cennzx": ["addLiquidity", "buyAsset", "removeLiquidity", "sellAsset", "setFeeRate"],
        "staking": ["bond", "bondExtra", "setController", "nominate", "unbond", "withdrawUnbonded", "setPayee", "rebond", "chill"],
        "nft": ["auction", "auctionBundle", "bid", "burn", "burnBatch", "buy", "cancelSale", "createCollection", "migrateToMetadataScheme", "mintAdditional",
        "mintSeries", "registerMarketplace", "sell", "sellBundle", "setOwner", "setSeriesName", "transfer", "transferBatch", "updateFixedPrice"],
            ...
            sections.map(async (section) => {
                ...
                await Promise.all(
                    mtds.map(async (mtd) => {
                        ...
                        console.log(`successfulExt of type ${section}:: is ${successfulExt}`);
